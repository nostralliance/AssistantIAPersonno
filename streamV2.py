...
# Définition du chemin du fichier de stockage des questions sans réponse
file_path = "stock_quest.txt"
...
# Fonction pour sauvegarder une question sans réponse dans un fichier texte
...
def generate_response(user_input: str):
    ...
    if intent is None:
        save_unanswered_question(user_input)  # Sauvegarde dans le fichier texte
...
# Bouton pour afficher le contenu du fichier stock_quest.txt
if st.button("Afficher le contenu de stock_quest.txt"):
    if os.path.exists(file_path):
        with open(file_path, "r", encoding="utf-8") as file:
            content = file.read()
        st.text_area("Contenu du fichier", content, height=200)
    else:
